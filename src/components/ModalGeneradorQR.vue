    ...
    <div class="bg-white dark:bg-dark-bg rounded-lg shadow-lg max-w-sm w-full">
      ...
      <div v-if="itemsProcesados.length > 0" class="p-6 text-center">
        ...
        <div class="flex justify-center p-4 bg-white rounded-lg">
          <QrcodeVue :value="itemsProcesados[0].valor_qr" :size="256" level="H" />
        </div>
      </div>
      ...
      <div class="flex gap-2 justify-end border-t border-gray-300 dark:border-gray-600 p-6">
        ...
        <button @click="downloadCSV" title="Descargar datos en CSV"
...
const itemsProcesados = computed(() => {
  ...
  let processedItem = {};
  if (props.isBienes) {
    // Obtenemos código base y secuencia
    const baseCode = rawItem.codigo || rawItem.bien_codigo || props.value || 'S/N';
    const sec = rawItem.bien_sec_alfabetica || '';

    processedItem = {
      original: rawItem,
      titulo_principal: rawItem.nombre || rawItem.bien_descripcion || props.title || 'Sin Descripción',
      codigo_visible: baseCode + sec,
      valor_qr: baseCode // Asumiendo que el QR también debe llevar la letra
    };
  } else {
    processedItem = {
      original: rawItem,
...
const downloadCSV = () => {
    ...
    if (props.isBienes) {
        ...
        rowData = [
            currentItem.original.bien_codigo || currentItem.original.codigo || 'S/N', 
            currentItem.original.bien_sec_alfabetica || '', 
            currentItem.titulo_principal,
            currentItem.original.bien_caracteristicas || currentItem.original.descripcion || '',
            currentItem.original.oficina || ''
        ];
    } else {
