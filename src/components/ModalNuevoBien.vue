<template>
  <div v-if="show"
    class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 transition-opacity duration-300 ease-out backdrop-blur-sm"
    @click.self="closeModal">

    <div
      class="bg-white dark:bg-dark-bg rounded-lg shadow-lg max-w-2xl w-full transition-transform duration-300 ease-out">
      ...
      <div v-if="step === 1" class="p-6 space-y-4">
        ...
        <div v-if="searchError"
          class="p-3 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 rounded-lg text-sm">
          {{ searchError }}
        </div>
        ...
        <div v-if="searchResults.length > 0" class="space-y-2 max-h-[30vh] overflow-y-auto">
          ...
          <button v-for="item in searchResults" :key="item.camb" @click="selectItem(item)"
      ...
      <div v-if="step === 2" class="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
        ...
        <div class="space-y-3 p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50 border dark:border-gray-600">
          <div class="flex justify-between items-center">
            ...
            <button @click="resetSearch" class="text-sm text-blue-600 dark:text-blue-400 hover:underline">Volver a
              buscar</button>
          </div>
          <div v-if="formError"
            class="p-3 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 rounded-lg text-sm">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input type="text" :value="`CUCOP: ${selectedCatalogoItem.clave_cucop}`" disabled
            ...
            <input type="text" :value="`CAMB: ${selectedCatalogoItem.camb}`" disabled
              class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white">
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Descripción (Nombre) <span
                class="text-red-500">*</span></label>
            <textarea v-model="newBienData.bien_descripcion" rows="3" placeholder="Descripción del bien (editable)"
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Características
              Detalladas</label>
            <textarea v-model="newBienData.bien_caracteristicas" rows="3"
              placeholder="Ej. Procesador i7, 16GB RAM, Color Negro..."
              class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"></textarea>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Cantidad a Registrar <span
                class="text-red-500">*</span></label>
            <input v-model.number="newBienData.cantidad" type="number" min="1" placeholder="Ej. 1"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="newBienData.bien_marca" type="text" placeholder="Marca del bien"
                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Modelo</label>
              <input v-model="newBienData.bien_modelo" type="text" placeholder="Modelo del bien"
                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            </div>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Número de Serie
              (Opcional)</label>
            <input v-model="newBienData.bien_serie" type="text" placeholder="Número de serie de fábrica"
              class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
          </div>
          <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Secuencia Alfabética</label>
              <input v-model="newBienData.bien_sec_alfabetica" type="text" maxlength="5" placeholder="Ej. A, B, AA"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Valor Monetario (por
                unidad)</label>
              <input v-model.number="newBienData.bien_valor_monetario" type="number" min="0" placeholder="Ej. 15000"
                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Número de Factura</label>
              <input v-model="newBienData.bien_numero_factura" type="text" placeholder="Factura"
                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            </div>
          </div>
          ...
          <div>
            ...
            <input v-model="newBienData.bien_proveedor" type="text" placeholder="Nombre del proveedor"
              class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select v-model="newBienData.bien_estado"
                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
      ...
      <div class="flex gap-2 justify-end border-t border-gray-300 dark:border-gray-600 p-6">
        ...
        <button @click="handleSave" :disabled="step === 1 || isSubmitting"
          class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors font-medium disabled:opacity-50">
...
const handleSave = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      bien_caracteristicas: formData.bien_caracteristicas || 'SIN CARACTERÍSTICAS',
      ...
      bien_sec_alfabetica: formData.bien_sec_alfabetica || null,
    };
