<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <label class="text-2xl font-bold text-gray-900 dark:text-white font-audiowide tracking-wide">Gestores</label>
    ...
    <div v-if="isLoading" class="flex items-center justify-center h-64 bg-white dark:bg-dark-bg rounded-lg shadow-md dark:shadow-stone-950 border border-gray-100 dark:border-gray-800">
    ...
    <div v-else-if="error" class="p-6 bg-red-100 dark:bg-red-900 rounded-lg text-red-700 dark:text-red-200">
        ...
        <button @click="fetchGestoresData(currentPage)" class="mt-4 px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">
            Reintentar
        </button>
    </div>
    ...
    <div v-if="showNewGestorModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
    ...
    <div v-if="showEditGestorModal && editingGestor.id" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
...
const fetchGestoresData = async (page = 1) => {
    ...
    try {
        ...
        if (term) {
            // Enviamos el término tal cual. Es responsabilidad de la API 
            // convertirlo a mayúsculas o usar 'ILIKE' para buscar.
