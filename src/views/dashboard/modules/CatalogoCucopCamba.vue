<template>
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <label class="text-2xl font-bold text-gray-900 dark:text-white font-audiowide tracking-wide">Catálogo CUCOP/CAMB</label>
            ...
            <div class="flex flex-col md:flex-row gap-4 items-end">
                <div class="flex-1">
                    ...
                    <input 
                        v-model="searchTerm"
                        type="text" 
                        placeholder="Buscar por Clave CUCOP, CAMB, Descripción o Partida..."
                        class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                    >
                </div>
                <button 
                    @click="openNewModal"
        ...
        <div v-if="isLoading" class="flex items-center justify-center h-64 bg-white dark:bg-dark-bg rounded-lg shadow-md border border-gray-100 dark:border-gray-800">
            <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-t-2 border-blue-600"></div>
            <p class="ml-4 text-gray-600 dark:text-gray-400">Cargando catálogo...</p>
        </div>

        <div v-else-if="fetchError" class="p-6 bg-red-100 dark:bg-red-900 rounded-lg text-red-700 dark:text-red-200">
            <h3 class="font-bold">Error al cargar el catálogo</h3>
            <p>{{ fetchError.message }}</p>
            <button @click="fetchCatalogo(currentPage)" class="mt-4 px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">
                Reintentar
            </button>
        </div>

        <div v-else class="bg-white dark:bg-dark-bg rounded-lg shadow-md dark:shadow-stone-950 overflow-x-auto border border-gray-100 dark:border-gray-800">
            
            <div v-if="catalogoList.data.length === 0" class="flex items-center justify-center h-64">
                <p class="text-center text-gray-500 dark:text-gray-400 text-lg font-medium">No se encontraron registros que coincidan con la búsqueda.</p>
            </div>
            ...
            <div v-else>
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                        <tr v-for="item in catalogoList.data" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
    ...
    <div v-if="showNewModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white dark:bg-dark-bg rounded-lg shadow-lg max-w-md w-full">
            ...
            <div v-if="newError" class="mx-6 mt-4 p-3 bg-red-100 text-red-700 rounded text-sm">{{ newError }}</div>
            ...
            <div class="flex gap-2 justify-end border-t border-gray-300 dark:border-gray-600 p-6">
                ...
                <button @click="saveNewItem" :disabled="isSubmitting" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50">{{ isSubmitting ? 'Guardando...' : 'Guardar' }}</button>
            </div>
        </div>
    </div>
    ...
    <div v-if="showEditModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white dark:bg-dark-bg rounded-lg shadow-lg max-w-md w-full">
            ...
            <div v-if="editError" class="mx-6 mt-4 p-3 bg-red-100 text-red-700 rounded text-sm">{{ editError }}</div>
            <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Clave CUCOP *</label>
            ...
            <div class="flex gap-2 justify-end border-t border-gray-300 dark:border-gray-600 p-6">
                ...
                <button @click="saveEditItem" :disabled="isSubmitting" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50">{{ isSubmitting ? 'Guardando...' : 'Guardar Cambios' }}</button>
            </div>
        </div>
    </div>
...
// --- FETCH (Lógica de Búsqueda Mejorada) ---
...
watch(searchTerm, () => {
    ...
    // No ponemos isLoading=true aquí para no "parpadear" la barra de busqueda, 
    // el spinner se activará dentro de fetchCatalogo
...
// IMPORTANTE: Eliminamos filteredCatalogo.
// Ahora la vista itera directamente sobre catalogoList.data que viene filtrado del servidor.
...
const saveEditItem = async () => {
    ...
    if (!editingItem.value || !editingItem.value.id) {
        editError.value = 'Error interno: No se pudo identificar el registro.'
        return
    }
